...
- **Channel content stopped loading entirely** (`ChatView.tsx`, since
  1.79.0): `setStrandedRecoverAttempts` was called inside the `messages`
  resource's fetcher, but the signal itself was declared later in the file.
  Solid's `createResource` runs its fetcher SYNCHRONOUSLY up to the first
  `await` as part of setting up the resource — inline during the
  component's initial body execution, not deferred to a later microtask —
  so this threw `ReferenceError: Cannot access 'setStrandedRecoverAttempts'
  before initialization` on every channel open, and the uncaught rejection
  left the message list permanently empty. Fixed by moving the signal's
  declaration above the resource. Caught only by actually running the app —
  neither `tsc` nor the test suite renders real Solid components, so this
  shipped despite both passing clean.
...
- **Interactive message buttons** (protocol §3.3, `sdk-js` 0.61.0+): any
  ...
  client's default feed, new-message scroll counter, and in-page quick-search
  (a convenience filter over the visible feed, not the spec's dedicated
  "search results" surface — see Fixed for what MUST stay unconditional:
  permalinks), so only the bot's reply becomes visible, letting a bot author
  build Telegram-style quick-reply menus (price/chart timeframe pickers,
  poll options, sub-menus)
...
- A channel page consisting entirely of button-press messages (e.g. many
  ...
  state once there's nothing older left to fetch. The in-page search "no
  matches" empty state is unaffected.
...
- **Accounts shows each wallet's display name and avatar ahead of its
  address.** A bare `klv1…` string is hard to tell apart from another at a
  glance — the name and picture are what a user actually recognises their own
  accounts by. Resolved through the same `resolveProfile` cache
  `BookmarksView`/`NewsView` use, so it costs no second lookup path, and falls
  back to two-letter initials when there is no avatar or no profile at all.
...
- The PIN confirmation now states the KLV amount. It previously read only
  "Smart contract call: register" — with no figure — on the first
  value-bearing contract call the app has ever shipped.
...
- **Editing an existing news post never restored its tags or attachments.**
  `ComposeView.tsx` decoded `post.payload` via `tryDecodeBase64Payload()`, on
  the assumption it was a base64 string — that assumption was always wrong;
  the L2 node sends `payload` as a raw byte array (`number[]`), which this
  file's own `getPayloadTitle`/`getPayloadContent` calls one line above
  already handled correctly. Surfaced by the `@ogmara/sdk` 0.51.0 type fix
  (`Envelope.payload` is now correctly typed as `number[]`, not `string`) —
  this file no longer compiled once the lie was corrected. Now decodes
  `payload` directly via `decodePayload()`, same as the rest of the file.
...
- **Clicking an image in a news post opened a new tab instead of the zoom
  ...
  Worse here than on the web build: a new tab is at least functional in a
  browser, whereas in the Tauri shell it is not the app's own viewer at all.
...
- **Cross-node media retry budget (45s) was sometimes shorter than the
  ...
  full retry window as an unhandled-rejection console error, then decoded
  fine after switching channels and returning — same CID, same key, it just
  needed more time). Raised the budget to 3 minutes and the poll interval
  to 5s (fewer redundant requests over the longer window) in
  `src/lib/mediaCrypto.ts`.
...
- **Media encryption gate failed OPEN: private/encrypted-channel images could
  ...
  DMs were verified independently: `attachEncryptedFile` calls the encrypted
  upload path unconditionally with no resource dependency, so no equivalent
  race exists there.
...
- **DM attachment picker intermittently did nothing — no error, just silence.**
  ...
  async native file dialog resolving (particularly flaky on Tauri's
  WebKitGTK on Linux), the element could get garbage-collected before
  `onchange` ever fired — the attach handler was simply never called, with
  nothing to catch or report. The channel composer never had this bug
...
- **Delete a DM conversation.** Right-click a conversation in the DM list for
  ...
  SDK method, and no protocol-level message type. Cross-device merge is a
  simple per-peer `max()` (mirrors web's identical implementation).
...
- **Deleting a channel now propagates across nodes** (via sdk-js 0.35.0):
  ...
  reconcile-indexes, so a deleted channel no longer resurrects on other nodes or
  shows in their channel list / search. Requires l2-node 0.76.0.
...
- **Editing/deleting a DM with an unverified wallet now shows a "verify your
  ...
  to the wallet page). This corrects 1.36.2, which dropped the client gate and let
...
- **DM messages now have a right-click / long-press context menu** (parity with
  channel chat), exposing Edit / Delete / quick-react. The 1.36.0 DM-edit feature
...
- **Late-joining device gets the key without an app reload.** `coverDevices` is now
  re-runnable (throttled ~10 s) and triggered on each incoming DM (`coverPeerDevices`):
  an active peer's newly-registered device gets the sender's conv_key automatically,
  instead of staying "🔒 waiting for key" until a restart.

## [1.35.2] - 2026-06-13

### Fixed

- **New incoming DM conversations now appear in the list immediately** (instead of
  only after a tab-switch). The conversation list refetches on the live `dm` WS
  event and on each unread poll while the DMs tab is open.
...
- **Cross-node DMs "can't decrypt"** — per-sender keys (mirrors web 0.47.4):
  decrypt a message by fetching its author's key; cache by (conversation, epoch,
  author). Needs l2-node 0.65.0 + sdk-js 0.29.0.
...
- DM conversation list shows **"🔒 Encrypted message"** for encrypted conversations
  instead of a blank/`...` preview. Mirrors web 0.47.2. (The garbled-ciphertext preview itself is
...
E2E encryption P1 — encrypted Direct Messages (text). Mirrors web 0.47.0.

### Added

- **`lib/dmCrypto.ts`** — DM conv_key lifecycle (establish/fetch/unwrap/cache,
  encrypt/decrypt) over `ChannelKeyEnvelope` (0x61). Uses the built-in wallet to sign
  and the random stable per-install `device_id`.
- DM send **encrypts text** (`buildEncryptedDm`); decrypt-on-render with
  "🔒 waiting for key…" / "🔒 can't decrypt" states (i18n in all 7 locales).
  Multi-device: keys wrapped to every bound device.

### Changed

- DM attachments are **blocked** (with a clear message) until encrypted media ships
  (P5/D6) — sending one would have downgraded the whole message to plaintext (audit
  2026-06-11 Sec-W1). Legacy plaintext DMs (`key_epoch 0`) still render.

### Notes

- conv_keys cached in-memory (per session); cross-device/restart recovery is P3.
  DM edit/delete content E2E is a P1 follow-up.
...
- **Block-explorer links now point to the Ogmara explorer (kleverchain.org)**
  instead of Kleverscan. The tip-sent links (NewsView, NewsDetailView) and the
  token portfolio's asset (`↗`) and transaction links now resolve through the
  new explorer. Links are built with a small `klever-explorer-links` helper
  (`ex.tx()` / `ex.wallet()` / `ex.asset()`) so URL shapes live in one place.
  The Ogmara explorer auto-detects mainnet vs. testnet (falling back to testnet
  when a hash isn't found on mainnet), so the per-network
  `testnet.kleverscan.org` switch — and the `getExplorerUrl()` helper — are
  gone; a single base URL covers all networks.
...
- **Right-click "mark as read" on DM conversations** in the sidebar — mirrors the
  channel context menu, with an optimistic unread-badge clear.
...
- **DMs appear without reopening the conversation.** The open conversation now polls
  the recipient's own authenticated endpoint every 8s (the node can't push DM bodies
  over the shared WS broadcast without leaking them to all clients). Dedup now drops
  a sent message's optimistic `local-` copy when the real one arrives, so polling no
  longer duplicates sends. The DM conversation list polls every 12s (new
  conversations appear) and the unread-badge poll sped up 30s -> 12s.
...
- **Cross-node DMs now appear in the open conversation.** The DM view had no polling
  ...
  *notification* (the unread badge) over WS — never the message content. So a
  received DM updated the sidebar count but the open conversation didn't fetch it.
  The DM view now polls the recipient's own authenticated DM endpoint every 10s.
...
§10.8). Lands alongside l2-node v0.48.0 + sdk-rust/js + website
...
- **`defaultNodeUrl` setting** — user-pinned "always connect here
  ...
  natively at a specific node — pinning guarantees the desktop
  app always lands there (including LAN nodes via the
  `allowPrivateHosts` opt-in that the desktop already uses).
...
- **`switchNodeSilent(url)`** — public helper used by bootstrap.
  ...
  the boot-time switch doesn't restart the webview mid-init.
